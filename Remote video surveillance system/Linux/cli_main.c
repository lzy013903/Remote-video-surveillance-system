...
button_area_draw(struct entry_win *c, GtkWidget* box)
{
	...
	/*连接按键*/
	button = gtk_button_new_with_label("连接摄像头");  
    gtk_box_pack_start_defaults(GTK_BOX(box), button);
    c->connect_button = button;
	//信号butoon的clicked，回调函数为button，参数是c
    g_signal_connect(button, "clicked", 
                        G_CALLBACK(connect_handler), c);		
...
static gboolean info_area_draw(struct wcam_win *c, GtkWidget *box)
{
    ...
    /* VERSION & HOMEPAGE */
    label = gtk_label_new("Github  ");
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(table), label, 0, 1, 0, 1,
                     GTK_FILL, GTK_SHRINK, 1, 1);

    label = gtk_link_button_new_with_label(WCAM_HOMEPAGE, "https://github.com/lzy013903");
